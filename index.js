...
function sendPictures(){
	// mars 621705786254426124
	var msg = ''
	const rand = getRandom(10, 39);
	const imageFile = fs.readdirSync(`./images/`).filter(file => file.endsWith(`${rand}.jpg`));
		msg += flowerFile[0].slice(0, imageFile[0].length - 7) + ' ';	
	...
	/* client.users.fetch('621705786254426124', false).then((user) => {
	user.send(newEmbed)
	user.send({embeds: [newEmbed], files: [attachment]});
	}); */
...
client.once('ready', () => {
	...
	//1 day = 86400000ms
	sendPictures();
	let scheduledMessage = new cron.CronJob('00 30 10 * * *', sendPictures);
